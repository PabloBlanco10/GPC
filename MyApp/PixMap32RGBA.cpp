...
void PixMap32RGBA::create_pixmap(GLsizei width, GLsizei height) // throw(std::bad_alloc)
{
    if(width != width_ || height != height_)
    {
        ...
        {
            throw std::runtime_error("PixMap32RGBA::create_pixmap() ERROR: Could not allocate memory (bad_alloc)");
        }
        if (data_ == nullptr)
        {
            width_  = 0;
...
void PixMap32RGBA::load_bmp24BGR(const std::string & file_name) // throw(std::ios_base::failure, std::bad_alloc)
{
    ...
    if (!stream.is_open())
    {
        throw std::ios_base::failure("PixMap32RGBA::load_bmpBGR() ERROR: Could not open file");
    }
...
void PixMap32RGBA::save_bmp24BGR(const std::string& file_name) const  // throw(std::ios_base::failure)
{
    ...
    if (!stream.is_open())
    {
        throw std::ios_base::failure("PixMap32RGB::save_bmpBGR() ERROR: Could not open file");
    }
...
PixMap32RGBA::PixMap32RGBA(const unsigned int width, const unsigned int height) // throw(std::bad_alloc)
...
PixMap32RGBA::PixMap32RGBA(const PixMap32RGBA & pixmap) // throw(std::exception)
: data_(nullptr), width_(0), height_(0)
{
    *this = pixmap;
}
...
PixMap32RGBA & PixMap32RGBA::operator=(const PixMap32RGBA & pixmap) // throw(std::exception)
{
    if (this != &pixmap)
    {
...
void PixMap32RGBA::set_alpha(GLubyte alpha) // throw(std::bad_alloc)
{
...
void PixMap32RGBA::set_colorkey_alpha(rgba_color colorKey, GLubyte alpha )
{
    if (data_ == nullptr) return;
    else
    {
